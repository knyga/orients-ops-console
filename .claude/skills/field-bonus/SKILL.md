...
- **Fund split (rules doc: «коли 3 людини — ділити на усіх»; restated 2026-09-05):** the day's fund is TWO per-person rates (pot = 2 × (700 + early + weekend)) shared by everyone bonus-counted on the report: each person's day amount = rate × `2 / max(2, N)` (`days[].splitFactor`; `days[].paidRoster` lists who shares). A lone pilot gets half the fund (one rate, e.g. 1200 of a 2400 early-Sunday pot — never the whole pot); two pilots get one rate each; 3+ divide the pot. An approver `not_counted` exclusion shrinks N, raising the remaining shares back toward full. Split is per **Звіт**, not per date. Person `gross`/`net` round once at period level, so expect ±1 ₴ vs naive per-day sums.
...
As each flight day's acceptance settles as ACCEPTED / ACCEPTED_EXCEPTION, DM
each participant their **provisional** share (the monthly drone-loss multiplier
settles separately). **Money is DM-only (since 2026-09-05):** nothing is posted
in the #field-qa verdict thread — not a breakdown, not a «не нараховано» note.
The thread is where the approvers' signals get processed; amounts stay private.

- Dry-run: `npm run field-bonus -- --start … --end … --notify` (prints, sends nothing).
- Send: add `--publish` (needs `chat:write`, `im:write`).
- Idempotent via the `bonus_notified` table (its `thread_ts` column is legacy — the breakdowns posted before 2026-09-05); only accepted, already-`field-publish`ed days are DMed; names without a Slack id are skipped and flagged (add them to `SLACK_ID_OVERRIDES` in `lib/fieldSlackIds.ts`).
