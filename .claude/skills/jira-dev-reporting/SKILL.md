...
Add `--summarize` (implies `--write`) to fill the `summary` column with a per-user occupation summary generated by Claude (claude-opus-4-8) from the titles of that user's resolved issues — useful for "what did each person work on?" questions. It needs `ANTHROPIC_API_KEY` set in `.env`; without it the CLI exits non-zero. Without `--summarize`, the `summary` column is present but empty.
