...
- [ ] **Step 1: Apply the migration** — `npm run db:migrate` (against Neon via `.env`). Verify: `select conname from pg_constraint where conrelid='published'::regclass and contype='p';` → `published_period_verdict_key_pk`; `select count(*) from published where verdict_key is null;` → 0.
...
- [ ] **Step 4: Strike the stale 07-01 message manually.** The legacy published entry (`period 2026-07`, `date 2026-07-01`, `verdict_key 2026-07-01`) points at the wrong ⛔ message. Edit that Slack message (via a one-off `updateMessage` call or Slack UI) to `~<old text>~\nЗастаріле — актуальні вердикти за 01.07 нижче в каналі (по одному на виїзд).` Do NOT delete the DB row (it anchors the old thread's `findPublishedByTs`).
