...
## Amendment 2 (2026-09-04, links-overridden-verdicts follow-up)

**Finding:** Amendment 1's carve-out (never edit an overridden verdict's message) was itself avoidable. Every edit of a bot message — a post, an approval strike, a roster correction, a links edit, a backfill — goes through `lib/slack.ts` `updateMessage`/`postMessage` → `sendTracked` → an `outbound_messages` row with `kind: "edit"`, the target's Slack `ts`, and the EXACT text sent, `status: "sent"`. So the live Slack text of any bot message is recoverable without touching `published.text` at all: it is the text of the newest `status = "sent"` row sharing that `ts` (the original post, or any later edit row, each recorded under its own idempotency key). `lib/dayLinks.ts` already had this exact primitive (`latestTextForTs`) for the reminder/bonus/Звіт-reply nodes — it just wasn't applied to the verdict node.

**Decision:** supersedes Amendment 1's carve-out.
- A tiny new pure-ish helper, `lib/liveText.ts` `liveVerdictText(entry)`, returns that live text (falling back to `entry.text` when no outbound row shares the ts yet).
- `lib/dayLinks.ts` `collectDayNodes` now sets a report's `verdictText` from `latestTextForTs` over the ts's own rows (dropping the `verdictOverridden` field entirely — nothing downstream needs it any more), so `planRelink` no longer special-cases an overridden report: it edits the verdict's LIVE text like any other target. The 🔗 line lands correctly whether or not an override has ever happened, and the strike is never clobbered because the "current text" the edit is built from already includes it.
- `lib/relinkDay.ts` `planRelinkForPeriod` now also resolves each published entry's own `findSentByTs(entry.ts)` (in addition to the existing feature-scoped reads) so a plain `approval`/`roster`-feature edit row (not just a `links`-feature one) is visible to the planner. `relinkDays`' TOCTOU guard and post-edit write-back both switch from comparing/writing `published.text` to comparing against `liveVerdictText(fresh.entry)` — and the write-back is now gated on `fresh.entry.override == null`, so an overridden verdict's `published.text` stays exactly as pristine as Amendment 1 required; the struck+relinked text lives only in the links edit's own outbound row.
- The two region editors that rebuild a *different* disjoint region on the same message — `lib/applyApproval.ts` `amendPublishedVerdict` and `lib/applyRosterCorrection.ts` `applyRosterDecision` — now also read their "keep this part intact" tail from `liveVerdictText`, not `entry.text`: `amendPublishedVerdict` still strikes the BODY from the pristine `entry.text` (so a re-amend never double-strikes), but takes the roster/drone/links tail from the live text, so a crew correction or a 🔗 edit that landed **after** a prior override is no longer erased by a later re-amend. `applyRosterDecision` splits the live text outright (so an overridden day's crew edit preserves the existing strike) and only writes `published.text` back when `entry.override == null` — mirroring the links write-back rule.

**Net effect:** the 🔗-region rule from the top of this doc ("edited even when the verdict is approver-overridden") now holds uniformly, as originally intended, without the follow-up column Amendment 1 deferred — the outbound table was already that "log of what actually got sent."
