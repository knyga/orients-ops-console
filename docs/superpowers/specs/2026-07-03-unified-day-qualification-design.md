...
- **New `pendingDays[]`** on `BonusReport`: PENDING / NEEDS_REVIEW days that flew — `flew: d.airborneMinutes > 0 || !d.airborneReported || d.deployMin != null` (a known deploy window is flight evidence too, even at 0 extracted airborne minutes — see 06-26 in §6) — with `{ date, roster, status, reasons, amountAtStake }` (the gross the crew would earn if accepted). Not counted in `total`.
