...
- There is a **drone-loss multiplier** for deployments where the drone was lost; the classifier runs via Codex and scans the #field-qa thread + Vimeo video names for loss evidence.
...
- Set `ANTHROPIC_API_KEY` in `.env` — Codex classifies drone-loss records from #field-qa threads and video names.
