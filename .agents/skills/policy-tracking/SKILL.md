...
When asked to produce a month's compliance record, classify the `NEEDS_REVIEW` occurrences with **Codex sonnet subagents** (one per month), then feed the verdicts back through the CLI so the committed report carries them:
...
3. `npm run policy -- --start … --end … --verdicts-file <path> --format table` → the CLI merges the verdicts (no Codex call), prints the table, and writes `reports/policy/<period>.{json,csv}` with verdicts filled. `--verdicts-file` implies `--write`.
