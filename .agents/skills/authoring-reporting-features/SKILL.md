...
This repo's master pattern: **Codex is the product.** Every reporting feature is driven from a skill + CLI that produce **committed artifacts**, which the web merely renders. Follow this recipe so a new feature matches the existing three (Vimeo, Jira, GitHub).

## The non-negotiables (from AGENTS.md)
...
7. **Feature skill** `.Codex/skills/<x>-dev-reporting/SKILL.md`, cloned from `jira-dev-reporting/SKILL.md`.
