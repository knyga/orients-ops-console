...
description: Use when answering questions about field drone flight hours, or when asked to extract/refresh flight hours from the #field-qa Slack channel for a date range (e.g. "how many flight hours in June?", "pull May's flight hours from field-qa", "update the flight-hours input for last month"). Reads airborne time from the stats-bot daily summary (text when posted, else the image via Codex vision) and writes the input the field-ops reconciliation consumes.
---
...
- The CLI parses the **text** body deterministically (`lib/flightTextParse.ts`,
  no LLM, no download) when present, and only falls back to **Codex vision**
  (Codex-sonnet-4-6) on the image for older/image-only days. The Slack
